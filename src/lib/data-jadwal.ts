...
// 1. Pasaran Jawa (Pancawara) Dinamis
export function getJavanesePasaran(date: Date): string {
  const pasarans = ["Legi", "Pahing", "Pon", "Wage", "Kliwon"]
  const utcDate = Date.UTC(date.getFullYear(), date.getMonth(), date.getDate())
  const utcRef = Date.UTC(2024, 0, 1) // 1 Jan 2024 adalah Senin Pahing (index 1)
  const diffDays = Math.floor((utcDate - utcRef) / (1000 * 60 * 60 * 24))
  const pasaranIdx = (((diffDays % 5) + 1) % 5 + 5) % 5
  return pasarans[pasaranIdx]
}

// 2. Tanggal Hijriah Dinamis
export function getHijriahDateString(date: Date): string {
  try {
    ...
    const cleaned = raw.replace(/\b(AH|H|Hijri|Hijriah)\b/gi, "").trim()
    return `${cleaned} Hijriah`
  } catch {
    return "1448 Hijriah"
  }
}

// 3. Detail Lengkap Tanggal Otomatis
export interface FullDateDetails {
  dayName: string
  pasaran: string
  masehiStr: string
  hijriahStr: string
...
export function getFullDateDetails(customDate?: Date): FullDateDetails {
  ...
  const masehiStr = `${date.getDate()} ${months[date.getMonth()]} ${date.getFullYear()} Masehi`
  const hijriahStr = getHijriahDateString(date)

  // Hari Libur Nasional
  const holiday = getLiburIndonesia(date)

  // Periode Akademik
  const monthNum = date.getMonth() + 1
  const year = date.getFullYear()
  const isGanjil = monthNum >= 7 && monthNum <= 12
  const academicSemester = isGanjil ? "Ganjil" : "Genap"
  const academicYearShort = isGanjil
  ...
  return {
    ...
    masehiStr,
    hijriahStr,
    holidayName: holiday ? holiday.nama : undefined,
    academicSemester,
    academicYearShort,
  }
}
